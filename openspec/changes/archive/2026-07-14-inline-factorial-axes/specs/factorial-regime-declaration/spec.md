...
**Reason**: The named-axis indirection (`factors` block referenced by name from `factorials`) served only axis reuse across crossings, which no configuration uses. Inline per-crossing axes remove two error classes (unknown axis reference, unused factor warning) and make each crossing self-contained.
**Migration**: Move each referenced axis's level map from `factors` into the `rows`/`columns` keys of the crossing that referenced it, then delete the `factors` block. Optionally add a `label` to the crossing to preserve a human-readable title.
