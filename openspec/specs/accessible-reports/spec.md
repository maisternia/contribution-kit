# accessible-reports Specification

## Purpose

Define accessible report-layout requirements for contribution-kit markdown outputs,
including titled sections, plain-language descriptions, input echoes, readable
columns, and per-table conclusions while preserving existing mismatch-risk
guarantees.
...
`prediction_expr` inputs only in the sections that actually use them. The
Shapley section SHALL show the prediction/target formula and the
`prediction_expr` input, while the observed-error and mismatch-risk sections
SHALL show the observed `prediction` input and SHALL NOT repeat Shapley-only
inputs. To make these available at render time, `Estimator.assess()` SHALL
populate `AssessmentResult.metadata` with `target`, `prediction`,
...
#### Scenario: Inputs are scoped to the section that uses them
- **WHEN** `to_markdown()` renders the Shapley, error-regime, and mismatch-risk sections
- **THEN** the Shapley section echoes `target`, `prediction_expr`, and `score_mode`
- **AND** the error-regime and mismatch-risk sections echo `target` and observed `prediction`
- **AND** the error-regime and mismatch-risk sections do not repeat the Shapley-only formula and scoring-mode lines
...
presentation layer. The mismatch-risk table SHALL label its percentage columns as
`Regime mismatch rate` and `Rest mismatch rate`, and SHALL format supporting
counts in parentheses as `mismatch:matched` (for example `22:1`). The
mismatch-risk table SHALL omit hypotheses whose matching rows contain zero
mismatches, because those rows add no positive mismatch signal to the risk-focused
summary even if they still appear in the regime-contribution table.
...
- **THEN** the table headers include `Regime mismatch rate` and `Rest mismatch rate`
...
- **WHEN** `to_markdown()` renders mismatch-risk results for a hypothesis whose matching rows have zero mismatches
- **THEN** that hypothesis is omitted from the mismatch-risk table
- **AND** the same hypothesis may still appear in the regime-contribution table if it has a regime summary
