# coalition-scored-conditions Specification

## Purpose

Expose the Shapley game's characteristic function `v(S)` to regime conditions
and factorial axis levels through a `coalition_score(...)` DSL function, so
callers can name the coalition they mean instead of re-deriving it by hand
from raw columns. The function returns the same number the Shapley pass
consumes for that row and coalition, keeps out of the expressions that define
the game itself, and leaves specs that never call it byte-identical to their
